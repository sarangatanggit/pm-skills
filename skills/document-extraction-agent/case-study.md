# Case Study: Document Extraction Agent — Compensation Onboarding

## Context

This is the founding case study for the document extraction agent skill. The domain is
compensation onboarding at a fintech platform; the pattern — extract structured rows from
unstructured documents, validate against a strict schema, flag rather than guess — applies
broadly to any domain where record-dense documents need to become import-ready data.

---

## The problem

White-glove client onboarding required reviewing 100+ contracts, offer letters, and
compensation schedules per client and converting them into a strict import spreadsheet —
one row per payout event, ready to load into the platform.
...
A document extraction agent — built with Claude and deployed as a Claude skill — that
ingests a folder of mixed compensation documents and produces a platform-ready import file
plus an exceptions report.
...
auditable. The rules became an artifact that could be read, reviewed, and updated independently
of the prompt.
...
1,200+ import rows. The extraction logic had to be explicit about what "recurring" meant for
each frequency type, what to do with open-ended schedules, and how to handle edge cases like
...
## How this instantiates the meta-skill

**Pattern Extraction:** Before writing prompts, identify the three payout types (one-time,
recurring, ad hoc) and their distinct expansion rules. These are the "record types" of this
domain — each requires different logic to produce the correct number of output rows.
...
file or exceptions report. The extraction logic tracks this state explicitly for every
...
This case study used compensation documents as the source domain, but the extraction
protocol is domain-agnostic. The same skill structure applies to:

- **Lease and property records** — extract tenant, unit, rent, term, and escalation clauses
  into a property management import
- **Vendor contracts** — extract service scope, fees, payment terms, and renewal dates into
  a procurement system
- **Legal filings** — extract party names, case numbers, dates, and obligations into a
  matter management tool
- **Employee records** — extract personal data, role history, and benefit elections into an
  HRIS import

In each case, the inputs change (schema, entity type, record types, expansion rules) but
the protocol is the same: classify documents, extract entities, expand records, validate
against schema, flag rather than fill.
